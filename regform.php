...
$_SESSION['ENROLLMENT_TYPE'] = $_POST['ENROLLMENT_TYPE'];
...
if($userStud){
	message(" is already taken.", "error");
    redirect(web_root."index.php?q=enrol");if ($userStud) {
    message("Username '" . $_SESSION['USER_NAME'] . "' is already taken.", "error");
    ...
    if ($age < 15){
       ...
       redirect("index.php?q=enrol");
		...
		if ($_SESSION['ENROLLMENT_TYPE'] == 'New') {
...
<!-- <form action="index.php?q=subject" class="form-horizontal well" method="post" > -->
    ...
    <!-- Add a new cell for the Enrollment Type label -->
    <td><label>Enrollment Type</label></td>
    <td colspan="2">
        <select class="form-control input-sm" name="ENROLLMENT_TYPE">
		
            <option value="New" <?php echo (isset($_SESSION['ENROLLMENT_TYPE']) && $_SESSION['ENROLLMENT_TYPE'] == 'New') ? 'selected' : ''; ?>>New</option>
            <option value="Old" <?php echo (isset($_SESSION['ENROLLMENT_TYPE']) && $_SESSION['ENROLLMENT_TYPE'] == 'Old') ? 'selected' : ''; ?>>Old</option>
            <option value="Transferee" <?php echo (isset($_SESSION['ENROLLMENT_TYPE']) && $_SESSION['ENROLLMENT_TYPE'] == 'Transferee') ? 'selected' : ''; ?>>Transferee</option>
        </select>
    </td>
</tr>
...
<td colspan="2">
    <select class="form-control input-sm" name="COURSE">
        ...
        if (isset($_SESSION['COURSEID'])) {
            ...
            echo '<option value="' . $singlecourse->COURSE_ID . '" selected>' . $singlecourse->COURSE_NAME . '-' . $singlecourse->COURSE_LEVEL . '</option>';
        } else {
        ...
        $allCourses = $mydb->loadResultList();
